...
function setTopAngVel(choice) {
    ...
    Body.rotate(topArm, changeAngle * choice, { x: sizeX / 2, y: sizeY / 2 });
    //stopBodies();
}
function setBotAngVel(choice) {
    let tVel = botArm.angularVelocity;
    console.log(`x: ${sizeY / 2 + 2 * padFactor * armLength * Math.cos(topArm.angle)}, y: ${sizeY / 2 + 2 * padFactor * armLength * Math.sin(topArm.angle)}`)
    Body.rotate(botArm, changeAngle * choice, { x: sizeY / 2 + 2 * padFactor * armLength * Math.cos(topArm.angle), y: sizeY / 2 + 2 * padFactor * armLength * Math.sin(topArm.angle) });
    //stopBodies();
}
...
function stopBodies() {
    totSystem.bodies.forEach(x => {
        ...
        x.angularSpeed = 0;
        x.constraintImpulse = { x: 0, y: 0, angle: 0 }
